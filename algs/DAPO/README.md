...
| 3 | **Degenerate groups** | Kept: a group where all completions score the same has zero advantage everywhere; its tokens still inflate the loss denominator and dilute the gradient | **Dynamic sampling**: such groups are removed from training | `rollout.dynamic_sampling` (auto-on for dapo); drop happens in the replay buffer, [`rollouts/replay_buffer.py`](../../rollouts/replay_buffer.py) |
...
Everything else — group z-score advantages baked into `zscore` at rollout time, uniform (non-group-structured) training batches, GA/DeepSpeed handling, micro-batch shuffling — is identical to GRPO; see the [GRPO README](../GRPO/README.md) for those mechanics. Two hard requirements beyond GRPO, both enforced at config load: **DAPO is sync-engine only** (`alg_name: dapo` with `overlap.enabled: True` is rejected) and **`reward.broadcast: True` is mandatory** — the token-level loss only means something if every response token carries the group advantage; with `broadcast: False` only the terminal prediction position would receive gradient.
...
rollout:
  n_samples: 8
  dynamic_sampling: null      # null = auto-enabled for dapo
...
- **Sync engine only.** Unlike GRPO, DAPO cannot run in overlap (async) mode at all: `alg_name: dapo` with `overlap.enabled: True` is rejected at config load (dynamic sampling and the overlong penalty are not wired into the async engine). Training batches are not group-structured, exactly as for GRPO — see the [GRPO README](../GRPO/README.md).
