...
- **Sync vs. async considerations**: PPO uses a **fixed** clip range $(1-\epsilon_\ell, 1+\epsilon_h)$ and a learned value function whose predictions become stale as the policy drifts. In sync mode each update sees near-on-policy data and the critic is fresh. In async / overlap mode, value estimates for older replay samples can drift from the current critic, and the fixed clip range does not self-adjust as ratios against $\pi_{\mathrm{old}}$ move outside $[1-\epsilon_\ell, 1+\epsilon_h]$. Overlap mode **auto-enables the decoupled loss** [[3]](#references) for PPO (identical mechanism to GRPO / CISPO; see [GRPO README: Decoupled loss (overlap mode)](../GRPO/README.md#decoupled-loss-overlap-mode) for the formula, proximal snapshot, and `overlap.behave_imp_weight_cap` knob), so the clip doesn't zero out gradients on stale tokens whose in-shard ratio is still close to 1. For heavy off-policy use you can also tighten the clip range, lower `train_steps_per_epoch` / `overlap.max_lag`, or switch to a critic-free algorithm with a data-driven clip such as [P3O](../P3O/README.md) [[4]](#references).
