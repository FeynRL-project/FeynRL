...
def _base_overlap_config(tmp_path):
    """Minimal valid RL config with overlap enabled (nccl sync, max_lag=2)."""
    return {
        "run": {
            "experiment_id": "test", "seed": 42, "project_name": "p",
            "tracking_uri": "", "checkpoint_dir": str(tmp_path),
        ...
        "deepspeed": {"zero_optimization": {"stage": 2}},
        "overlap": {"enabled": True, "max_lag": 2},
    }


def test_force_strict_overlap_max_lag_gt1_raises(tmp_path):
    """force_strict_on_policy=True + overlap.enabled=True + max_lag > 1 must be rejected."""
    cfg = _base_overlap_config(tmp_path)
    config_file = tmp_path / "config.yaml"
    with open(config_file, "w") as f:
        yaml.dump(cfg, f)
    with pytest.raises(ValueError, match="force_strict_on_policy"):
        load_and_verify(method="rl", input_yaml=str(config_file), experiment_id="e", rank=0)


def test_force_strict_overlap_max_lag_1_passes(tmp_path):
    """force_strict_on_policy=True is allowed when overlap.max_lag=1 (no staleness)."""
    cfg = _base_overlap_config(tmp_path)
    cfg["overlap"]["max_lag"] = 1
    config_file = tmp_path / "config.yaml"
    with open(config_file, "w") as f:
        yaml.dump(cfg, f)
    config = load_and_verify(method="rl", input_yaml=str(config_file), experiment_id="e", rank=0)
    assert config.overlap.max_lag == 1


def test_force_strict_false_overlap_max_lag_gt1_passes(tmp_path):
    """force_strict_on_policy=False with overlap.max_lag > 1 is a valid async config."""
    cfg = _base_overlap_config(tmp_path)
    ...
    with open(config_file, "w") as f:
        yaml.dump(cfg, f)
    config = load_and_verify(method="rl", input_yaml=str(config_file), experiment_id="e", rank=0)
    assert config.overlap.max_lag == 2
